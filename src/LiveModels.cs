...
namespace FStudioMcp {
 internal static partial class VisibleHost {
  ...
  sealed class ModelPropertyChange {internal string Ref,Member;internal object Target,Scope,Before,After;internal PropertyInfo Property;}
  ...
  static object SetModelProperties(Dictionary<string,object> a) {
   ...
   foreach(var entry in input){
    ...
    var target=Ref(change);RequireBackgroundModelTarget(target);
    var name=S(change,"member");var property=target.GetType().GetProperty(name,BindingFlags.Public|BindingFlags.Instance);
    ...
    if(changes.Any(c=>c.Ref==id&&c.Member==name))throw new ArgumentException("Duplicate property in batch: "+name);
    ...
    changes.Add(new ModelPropertyChange{Ref=id,Member=name,Target=target,Scope=scope,Property=property,Before=property.GetValue(target,null),After=ConvertArg(change["value"],property.PropertyType)});
   }
  ...
  static object ModelOperation(Dictionary<string,object>a){
   ...
   if(op=="read_member"){
    object value=a.ContainsKey("member")?Member(o,S(a,"member")):o;
